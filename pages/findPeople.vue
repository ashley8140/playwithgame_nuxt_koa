<template>
  <div class="container">
    ...
    <ul class="search">
      ...
      <li class="first">
        ...
        <ul class="list fl point" :class="{'moregame': moregame}">
          ...
          >
            ...
            <p
              class="game_name"
              :class="{'active': item.key == key || show == index}"
            >{{item.title}}</p>
          </li>
        </ul>
        <div class="more point" @click="more('moregame')" :class="{'up': moregame}"></div>
      </li>
      <li v-for="(item, index) in taglist" :key="index" class="tags">
        ...
        <ul class="list fl" :ref="item.key">
          ...
          <li
            v-for="(i, n) in item.lists"
            :key="n"
            @click="changeTag(item.key, i.id)"
            :class="{'active': tag[item.key] == i.id}"
          >{{i.value}}</li>
        </ul>
        <div class="more point" :ref="item.key+'more'" @click="more(item.key, item.key+'more')"></div>
      </li>
      <li class="tags">
    ...
    <section class="choose" v-show="gameinfo.length > 0">
      <div
        class="multiple item point"
        :class="{'active': orderby == 0}"
        @click="changeOrderby('mutiple')"
      >综合排序</div>
      <div
        class="number item point"
    ...
    <div class="game_list">
      <div class="notFound" v-show="gameinfo.length == 0">没有找到符合条件的人哦！</div>
...
export default {
  ...
  computed: {
    ...mapState("login", ["userInfo", "access_token"])
  },
  methods: {
