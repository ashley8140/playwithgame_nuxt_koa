<template>
    <div>
        <div class="p1 bold"><span @click="changeCur(0)"
                  :class="{'active': cur==0}">我的订单</span> | <span @click="changeCur(1)"
                  v-if="userInfo.stype==1"
                  :class="{'active': cur==1}"> 陪玩订单</span>
            <div class="filter"><span class="all s1"
                      :class="{'choose': type==0}"
                      @click="changeType(0)">全部</span><span class="s2"
                      @click="changeType(1)"
                      :class="{'choose': type==1}">待评价</span><span class="s3"
                      @click="changeType(2)"
            ...
            <ul class="list f14">
                <li class="item"
                    ...
                    <div class="detail flex">
                        ...
                        <!-- 用户取消订单 0 || 1-->
                        <div class="d5 d"
                             style="color:#FF4C2E;cursor: pointer;"
                             v-show="i.status==0 || i.status==1"
                             @click="show_cancel(i.id, index)"
                             v-if="cur==0">
                            取消订单
                        </div>
                        <!-- 待评价 3 -->
                        <div class="d5 d"
                             v-show="i.status==3&&i.is_comment==0"
                             style="color:#FF4C2E;cursor: pointer;"
                             @click="show_comment(i.id, i.uid_service, index)"
                             v-if="cur==0">
                            待评价
                        </div>
                        <!-- 用户 完成确认 2 -->
                        ...
                        <!-- 用户 -->
                        <div class="d5 d"
                             v-show="i.status==3&&i.is_comment==1"
                             style="color:#FF4C2E;cursor: pointer;"
                             @click="show_comment(i.id, i.uid_service, index)"
                             v-if="cur==0">
                            修改评价
                        ...
                        <!-- 用户 -->
                        <div class="d5 d"
                             v-show="i.status==3&&i.is_comment==2"
                             v-if="cur==0">
                            已完成
                        </div>
                        <!-- 猎人确认接单 1 -->
                        <div class="d5 d"
                             v-if="cur==1&&i.status==1"
                             style="color:#FF4C2E;cursor: pointer;"
                             @click="receive(i.id,index)">
                            确认接单
                        </div>
                        <!-- 猎人 完成确认 2 -->
                        <div class="d5 d"
                             v-show="cur==1&&i.status==2"
                             style="color:#FF4C2E;cursor: pointer;"
                        ...
                        <!-- 猎人 -->
                        <div class="d5 d"
                             v-show="cur==1&&i.status==3&&i.is_comment==1">
                            已评价
                        </div>
                        <!-- 猎人 -->
                        <div class="d5 d"
                             v-show="cur==1&&i.status==3&&i.is_comment==0">
                            待评价
                        </div>
                        <!-- 猎人 -->
                        <div class="d5 d"
                             v-show="cur==1&&i.status==3&&i.is_comment==2">
                            已完成
                        </div>
                    </div>
                    <nuxt-link tag=div
                               class="goto"
                               :to="{path: '/userCenter/orderdetail', query: {type: cur,id: i.id}}">
                        <img class="goto"
                             src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAkAAAAPBAMAAADE5vg+AAAAIVBMVEUAAAD/Sy7/SSj/TC3/RCL/TC3/RCv/Sy3/Rif/TC7/TC6zLWLIAAAACnRSTlMA3x/GD8AeqiH2FmP/aAAAACZJREFUCNdjaE1gAIIqIRDpvFABSLJY4eKwzQqAsMHiGEyYORAzAfurCyt7/4GLAAAAAElFTkSuQmCC"
                             alt="">
                    </nuxt-link>
                </li>
            </ul>
            ...
            <div class="box center_box">
                ...
                <div style="margin: 23px 0 30px;">
                    <span v-for="(item, index) in star"
                          :key=index
                          class="s"
                          :class="{'active':index <= s_l}"
                          @click="getStar(index)"></span>
                    <span class="s5 fr">{{score}}分</span>
                </div>
                <textarea placeholder="请输入对此服务的评价"
        ...
        <div class="mask cancelOrderBox"
             v-show="cancelOrderBox">
            <div class="pop"></div>
            <div class="box center_box">
                <p class="center t2"
                   style="margin-bottom: 70px;">确定要取消该订单吗？</p>
                <div class="yes d d1 fl"
                     @click="cancel">
                    确认
                </div>
                <div class="no d fr"
        ...
        <div class="mask"
             v-show="contactBox">
            <div class="pop"></div>
            <div class="box center_box f20 center"
                ...
                .then(data => {
                    ...
                    if (d.code == 200) {
                        ...
                        for (var i = 0, l = this.list.length; i < l; i++) {
                            this.list[i].addtime = utils.dataFormatDetail(
                                this.list[i].addtime
                            );
                        }
                    } else {
