<template>
    <div>
        <ul class="game">
            <li class="item"
                v-for="(item,index) in gameinfo"
                :key=index>
                <router-link :to="{name:'hunterInfo', params:{touid: item.user_id,key:game_key}}">
                    <img class="pic point"
                         :src=item.auth_avatar
                         alt="">
                </router-link>

                <span class="discounted"><img :src=base64.zk
                         alt=""></span>
                <span class="voice fl point"
                      @click="controlPlay('v'+item.user_id)">
                    <audio :src=item.voice
                           :class="'v'+item.user_id">
                        您的浏览器不支持
                        audio 标签
                    </audio>
                    <img v-show="item.sex==1"
                         src="../assets/imgs/yy_male.png"
                         alt="">
                    <img v-show="item.sex==2"
                ...
                <div class="footer">
                    <div style="overflow:hidden">
                        ...
                        <span class="fl"
                            ...
                            <img :src=level[item.lv_level]
                                 alt="">
                        </span>
                        <span class="price fr">¥ <span>{{item.min_price}}/小时</span></span>
                    </div>
                    <div style="overflow:hidden">
                        <span class="zs fl"><img v-show="item.sex==2"
                                 class="sex"
                                 :src=base64.female
                                 alt="女"><img class="sex"
                                 v-show='item.sex==1'
                                 :src=base64.male
                                 alt="">
                            <img v-for="(item, index) in v_level[item.v_level]"
                                 :key=index
                                 :src="base64[item]"
                                 alt="">
                        </span>
                        <span v-show="item.online_status==1"
                              class="status fl">在线</span>
                        <span v-show="item.online_status==0"
                              class="status fl">下线</span>
                    </div>
